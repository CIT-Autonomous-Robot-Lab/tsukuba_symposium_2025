...
% タイトル
\title{小型自律移動ロボットを用いたつくばチャレンジ2025での取り組み}

\author{○中村　啓太郎\authorrefmark{1}, 吉越　誠\authorrefmark{2}, 佐々木　新平\authorrefmark{2},
\\船井　涼\authorrefmark{2}, 川原　脩慈\authorrefmark{2}, 茂　郁良\authorrefmark{2},
\\水牧　鷹哉\authorrefmark{1}, 根本　光\authorrefmark{1}, 福澤　槙之介\authorrefmark{1}, 和田　純弥\authorrefmark{1},
...
\etitle{Development for Tsukuba Challenge 2025 with Small Mobile Robots}

\eauthor{○Keitaro NAKAMURA\eauthorrefmark{1}, Makoto YOSHIGOE\eauthorrefmark{2}, Shimpei SASAKI\eauthorrefmark{2}, Yuki NAGAKI\eauthorrefmark{2},
\\Ryo FUNAI\eauthorrefmark{2}, Shuji KAWAHARA\eauthorrefmark{2}, Ikuo SHIGE\eauthorrefmark{2},
\\Takaya MIZUMAKI\eauthorrefmark{1}, Hikaru NEMOTO\authorrefmark{1}, Kenta HIRACHI\authorrefmark{1}, Shinnosuke FUKUZAWA\eauthorrefmark{1}, Junya WADA\eauthorrefmark{1}, Tuvshinbayar LKHAGVA-OCHIR\eauthorrefmark{1},
\\Yasuo HAYASHIBARA\eauthorrefmark{1}, Ryuichi UEDA\eauthorrefmark{1}}

\affiliation{千葉工業大学 未来ロボティクス学科 上田研究室 むぎまるチーム/きなこチーム/しらたまチーム}
...
つくばチャレンジ2025には,
当研究室から「むぎまるチーム」, 「きなこチーム」，「しらたまチーム」
という名義で計3チームが参加した.
各チームは開発の目標を表\ref{table:team_goals}のように設定し活動した.
% むぎまるチームでは
% 価値反復アルゴリズムを用いた自律移動システムの
% 開発に注力し, 開発においての目標を
% 「価値反復アルゴリズムを用いたROSパッケージの
% 経路追従タスクへの適用」
% として活動した.
% きなこチームでは,
% 開発においての目標を「
% ベクトル量子化による3次元占有格子地図の圧縮と圧縮したままの地図を用いた自己位置推定の実施
% 」
% として活動した.
% しらたまチームでは,
% 開発においての目標を「
...
の詳細と, つくばチャレンジ2025における各チームの取り組みを述べる.
本稿の構成は次の通りである.
...
\begin{table}[H]
  \centering
  \caption{各チームの目標}
  \label{table:team_goals}
    \begin{tabular}{|l|p{6.5cm}|}
    \hline
    チーム   & 目標 \\
    \hline
    むぎまる & 価値反復アルゴリズムを用いたROSパッケージの経路追従タスクへの適用 \\
    \hline
    きなこ   & ベクトル量子化による3次元占有格子地図の圧縮と圧縮したままの地図を用いた自己位置推定の実施 \\
    \hline
    しらたま & ROS 2の基礎知識習得と実機での実装 \\
    \hline
  \end{tabular}
\end{table}
%\begin{itemize}
...
全チームで使用したロボットはアールティ社製のRaspberry Pi Cat\cite{RTshop}を改造したものである.
各チームのロボットの外観を図\ref{fig:robot}に示す.
しらたまチームは元のロボットに取手を取り付けた.
むぎまる，きなこ両チームのロボットは,
取手の取り付けに加え,
Raspberry Pi Catの外装を大型化した.
この改造により, メンテナンス性が向上し,
新しいセンサや追加のバッテリーなど
の装置を乗せることができるようになった．
実際，むぎまるチームは，バッテリーとminiPCを追加した．
一方で, モータや電源回路などは改造前のものをそのまま使用している.

% しらたまを追加すること
 \begin{figure}[htbp]
   \begin{center}
     \includegraphics[width=1.0\linewidth]{figs/robot_image.eps}
     \caption{各チームの機体}
     \label{fig:robot}
   \end{center}
 \end{figure}
...
Raspberry Piに加え
むぎまるチームはminiPC，
きなこチームとしらたまチームはノートPCを搭載している.
Raspberry Piでモータの制御など低レイヤーの処理を行い,
上位レベルの処理はノートPCやminiPCが担う.
...
\begin{table}[H]
  ...
  % \begin{tabular}{|l|p{4.0cm}|p{1.0cm}|}
    ...
    チーム   & CPU & RAM\\
    \hline
    むぎまる & AMD® Ryzen 9 7945HX3D & 64GB \\
    \hline
    きなこ   & 12th Gen Intel® Core™ i7-12700H  & 32GB \\
    \hline
    しらたま & 12th Gen Intel® Core™ i5-1235U & 16GB \\
    \hline
  \end{tabular}
\end{table}
...
% センサ構成としては, 全チーム
% Raspberry Pi Cat標準のエンコーダを使用している.
% LiDARには，むぎまるチームときなこチームは
% 3D LiDAR（Livox Mid-360），
% しらたまチームは，2D LiDAR（Hokuyo UTM-30LX），
% むぎまるチームはさらに, アールティ社の
% IMUセンサモジュール（RT-USB-9axisIMU2\cite{RTshopIMU}）を使用している．

全チームには, 外界センサとしてLiDARを搭載している.
むぎまるチームときなこチームは3D LiDAR（Livox Mid-360）,
しらたまチームは2D LiDAR（Hokuyo UTM-30LX）を使用している.

さらに, むぎまるチームときなこチームには, 内界センサに
エンコーダとIMUを搭載している.
エンコーダには両チームともRaspberry Pi Cat標準のものを搭載している.
IMUについて, むぎまるチームはIMUセンサモジュール（RT-USB-9axisIMU2\cite{RTshopIMU}）,
きなこチームはハードウェアの冗長性の排除を目的として3D LiDARに付属のもの（ICM-40609-D）を使用している.

ソフトウェアは,
全チームROS 2\cite{ROS 2}をベースに構築した.
ソフトウェアの詳細については,
次節以降で説明する.
...
\input{robot_shiratama}

\section{本走行の結果}

つくばチャレンジ2025での本走行の結果を表\ref{MainRun}に示す.
全チームが完走を達成できず, 表中の理由でリタイアとなった.
...
\begin{table}[H]
  ...
  \begin{tabular}{|c|c|p{4.0cm}|}
    ...
    チーム         & 走行距離 & リタイアの理由\\
    \hline
    むぎまるチーム & $88$ m    & 道を間違えリアイア \\
    \hline

    きなこチーム　 & $588$ m    & 一時停止位置において停止せず通過 \\
    \hline

    しらたまチーム　 & $141$ m    & 他チームの機体との衝突 \\
    \hline
  \end{tabular}
\end{table}
...
% むぎまるチームで使用した価値反復パッケージには,
% ロボットが走り出すまでの方策計算に時間をかけてしまう問題がある.
...
今年度全チームが自己位置推定で使用したMCLは,
途中で推定が破綻する問題がある.
この問題に対して, 本研究室では,
global navigation satellite system（GNSS）を使用して
自己位置推定の破綻を修復するアプローチを提案している\cite{吉越2024, 吉越2025}.
...
本年度は3チームが参加し,
自己位置推定やナビゲーションのソフトウェア開発,
ROS 2で移動ロボットを動かすための基礎知識の習得に取り組んだ.

むぎまるチームは,
価値反復アルゴリズムをロボットの経路計画に適用したパッケージを開発し,
つくばチャレンジの環境で動作させた.
本走行では,
途中まで価値反復によるナビゲーションができることを確認したが,
別のソフトウェアの動作不良が原因でリタイアした.

きなこチームは,
ベクトル量子化により,
大容量な3次元占有格子地図を圧縮し,
$30.9$GBの地図を$74.6$MBまで削減した(圧縮率$0.0024$).
また, 圧縮地図を用いた自己位置推定を行うため,
emcl2\_ros2を拡張して3次元点群に対応させ,
点群の高さを走行中に切り替えることで動的環境でのロバスト性を向上させた.
さらに, FMMに基づくベクトル場プランナーを開発し,
観測範囲全域を単一のポテンシャル場として扱うことで,
シンプルなシステムを実現しつつ,
対向ロボットや歩行者に対して余裕を持った回避行動を可能にした.
本走行では$588$mの走行を達成し,
その後も走行を継続して$852$mまで到達したが,
一時停止位置および信号機前で停止できずリタイアとなった.

しらたまチームは,
新たなソフトウェアの開発は行わなかったが,
ROS 2で移動ロボットにナビゲーションをさせる上で必要な要素を学習した.
本走行では, 他チームの機体と接触しリタイアとなったが,
ナビゲーションを開始するまでの手順を学ぶことができた.

今後は, 今年度の参加で得た知見や明らかとなった問題を基に
ソフトウェアの改良やシステム構成の見直しを行う.
それに加え, \ref{sec:others}章であげた取り組みを試し,
新たな知見を得ることを考えている.
これらの取り組みにより,
安定したナビゲーションシステムの構築を目指し,
来年度はコース全域での完走を目指す.
...
上田研究室の平地健太氏, ハグワーオチルトゥブシンバヤル氏, 永木悠暉氏および
林原研究室の皆様には, つくばチャレンジ2025の参加にあたりご意見, ご協力頂き感謝申し上げます.
...
\begin{thebibliography}{99}
  ...
  \bibitem{pc2pgm_github}
    cafeline: ``cafeline/pointcloud2pgm\_slicer,'' \url{https://github.com/cafeline/pointcloud2pgm_slicer} (last visit: 2026-01-27).
  ...
  \bibitem{船井2026}
	  船井涼, 林原靖男, 上田隆一: ``移動ロボットの自己位置推定のためのベクトル量子化による3次元占有格子地図の圧縮'', 第31回ロボティクスシンポジア, 2026（to appear）

  \bibitem{vq_occupancy_compressor_github}
    cafeline: ``cafeline/vq\_occupancy\_compressor\_slicer,'' \url{https://github.com/cafeline/vq_occupancy_compressor} (last visit: 2026-01-30).

  \bibitem{kinako_nav_github}
    cafeline: ``cafeline/kinako\_nav,'' \url{https://github.com/cafeline/kinako_nav} (last visit: 2026-01-30).

  \bibitem{sethian1996}
  J. A. Sethian: ``A Fast Marching Level Set Method for Monotonically Advancing Fronts,''
  Proc. Natl. Acad. Sci. USA, Vol. 93, No. 4, pp. 1591--1595, 1996.

  \bibitem{tsitsiklis1995}
  J. N. Tsitsiklis: ``Efficient Algorithms for Globally Optimal Trajectories,''
  IEEE Trans. Automatic Control, Vol. 40, No. 9, pp. 1528--1538, 1995.
  ...
  IEEE Trans. Systems Science and Cybernetics, Vol. SSC-4, No. 2, pp. 100--107, 1968.
...
%上田隆一, 登内リオン, 池邉龍宏, 林原靖男: ``移動ロボットのための自己位置の不確かさを考慮したセンシングできない固定障害物の回避手法 ---価値反復を用いたナビゲーションにおける状態空間の局所拡張---'', 第28回ロボティクスシンポジア講演論文集, pp. 118-123, 2023.
  ...
  \bibitem{dbscan}
  M. Ester, H.-P. Kriegel, J{\"o}rg Sander, and X. Xu:
  ``A Density-Based Algorithm for Discovering Clusters in Large Spatial Databases with Noise,''
  Proc. of the Second International Conference on Knowledge Discovery and Data Mining (KDD-96), pp. 226--231, 1996.
  ...
  SteveMacenski: ``SteveMacenski/slam\_toolbox,'' \url{https://github.com/SteveMacenski/slam_toolbox} (last visit: 2025-1-27).
   ...
   \bibitem{中村2025}
    中村啓太郎，林原靖男，上田隆一: ``自律移動ロボットのための価値反復と A*探索を組み合わせた大域経路計画 — 探索空間を 3 次元に拡張した A*との組み合わせの検討—''，第43回日本ロボット学会学術講演会(RSJ 2025)，3Q3-06，2025

  \bibitem{ikebe2023}
    池邉龍宏, 林原靖男, 上田隆一: ``未知障害物によるモンテカルロ自己位置推定の破綻を防ぐための観測範囲の制限と選択'', 日本機械学会ロボティクス・メカトロニクス講演会2023講演論文集, 2A2-G06, 2023

  \bibitem{吉越2024}
	  吉越誠, 池邉龍宏, 林原靖男，上田隆一: ``LiDARによる自己位置推定が破綻した際のGNSS測位可能な場所への自律移動ロボットの誘導'', ロボティクス・メカトロニクス講演会2024, 1P1-M08, 2024

  \bibitem{吉越2025}
	  吉越誠, 林原靖男, 上田隆一: ``移動ロボットのための彷徨しながらの衛星測位によるLiDAR ベースの自己位置推定の破綻からの復帰法とナビゲーションの継続'', 第30回ロボティクスシンポジア, pp.107-112, 2025
