...
各チームともアールティ社製のRaspberry Pi Cat\cite{RTshop}を
改造したロボットを使用した（図\ref{fig:robot}）.
視認性と緊急時の持ち上げやすさを考慮し,
すべてのロボットに取手を追加している.
また,
むぎまるときなこチームはフレームを大型化し,
メンテナンス性と搭載量を向上させた.
この改造により.
むぎまるチームは，バッテリーとminiPCを追加した．
その一方で, モータや電源回路などは改造前のものをそのまま使用している.
...
きなこチームは,
ベクトル量子化により3次元占有格子地図を圧縮し($30.9$GBを$74.6$MBまで削減),
圧縮地図を用いた3次元自己位置推定を実装するとともに,
利用する点群高さの切り替えにより動的環境でのロバスト性向上を実現した.
また,
圧縮前の地図容量より小さいメモリ容量（$4$GB）の計算機にも
圧縮地図を搭載して運用できることを確認した.
さらに,
FMMに基づく経路計画を実装し,
本走行では$588$m, 継続走行では$852$mまで安定した走行を実現した.
...
\begin{thebibliography}{99}
  ...
  \bibitem{vq_occupancy_compressor_github}
    cafeline: ``cafeline/vq\_occupancy\_compressor,'' \url{https://github.com/cafeline/vq_occupancy_compressor} (last visit: 2026-01-30).
