...
%他チームと同じくemcl2\_ros2を使用した.
 しらたまチームは, つくばチャレンジにおける開発目的であるROS 2の基礎知識習得を達成するため, 当研究室でメンテナンスされている emcl2\_ros2 を自己位置推定手法として採用した. 本チームでは
ソースコードには手を入れず, 
パラメータのみを変更して使用した. 

ナビゲーションにはROS 2標準のパッケージであるnavigation2\cite{nav2}を採用した. 
...
むぎまるチームときなこチームでも使用しているraspimouseノードがある. 
...
\item navigation2: ナビゲーションパッケージに属するノード群(図中では複数のノードをまとめて「navigation2」と表記)
\end{itemize}
がある. 
